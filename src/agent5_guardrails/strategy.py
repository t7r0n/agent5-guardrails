...
COMPANY = 'Open'
REPO = 'agent5-guardrails'
PROJECT_TERMS = ['thesis', 'regulated', 'enterprise', 'fintech', 'healthcare', 'resolving', 'issues', 'autonomously']
PROJECT_METRICS = ['thesis_coverage', 'regulated_risk', 'enterprise_precision', 'fintech_latency']
PROJECT_FAILURES = ['thesis_drift', 'regulated_gap', 'enterprise_misroute', 'fintech_blindspot']
PROJECT_ARCHETYPES = [{'name': 'thesis evidence replay', 'trigger': 'thesis signal changes while regulated context is stale', 'expected': 'block release until cited evidence is regenerated'}, {'name': 'regulated boundary probe', 'trigger': 'regulated handoff crosses a policy or trust boundary', 'expected': 'route to reviewer with evidence packet'}, {'name': 'enterprise regression harness', 'trigger': 'enterprise behavior regresses against the last accepted fixture', 'expected': 'open a regression issue with trace and benchmark delta'}, {'name': 'fintech operator packet', 'trigger': 'fintech output needs a human-readable audit packet', 'expected': 'accept only if decision claims cite fixture evidence'}]
PROJECT_DIRECTION = 'A live evaluation + policy conformance harness that turns Open\'s webhook stream into an auditable "Agent 5 didn\'t break the rules this week" dashboard, with zevals compatible regression suites synthesised from real handoffs.'
...
HERO_TITLE = 'Open Adversarial Replay Board'
